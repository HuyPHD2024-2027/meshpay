...
def fig_hop_count_vs_loss(runs: List[Dict], output_dir: Path) -> None:
    ...
    axes[-1].legend(fontsize=9, edgecolor="#BDBDBD", framealpha=0.9)
    fig.suptitle("Average Bundle Hop Count vs. Packet-Loss Probability", fontsize=13, fontweight="bold")
...
def main() -> int:
    ...
    print("Figure 6: Avg hop count vs. packet loss")
    fig_hop_count_vs_loss(runs, output_dir)

    print("Figure 7: Application goodput before/during/after attack")
    fig_bandwidth_phase_table(runs, output_dir)

    print("Figure 8: Application goodput at 50% packet loss")
